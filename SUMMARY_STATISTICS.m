%SUMMARY_STATISTICS: calculates complex modulus, loss, and storage modulus
%from a condition's spots file
...
for i = 1:length(SPOTS_FILES_ARRAY)
    ...
    %checkpoint to manually truncate data. data must be truncated first in order to be saved  
    while 1
...
writecell(summary_stats_orig, strcat(summary_folder_original, "/", CONDITION,  "_SUMMARY_STATS_ORIG.xlsx" ));
close all


cell2table(summary_stats_final);
writecell(summary_stats_final, strcat(summary_folder_final, "/", CONDITION,  "_SUMMARY_STATS_FINAL.xlsx" ));
close all
